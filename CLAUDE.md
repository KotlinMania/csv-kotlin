# Claude Code Project Instructions — csv-kotlin

## Project Overview

This is **csv-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`csv`](https://crates.io/crates/csv).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/csv/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/csv>
