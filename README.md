...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`csv`](https://crates.io/crates/csv).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `csv` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Fast CSV reader and writer
...
Original work copyrighted by the upstream `csv` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
